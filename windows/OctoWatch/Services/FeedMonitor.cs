...
internal sealed class FeedMonitor
{
    ...
    private void ApplyResult(FeedSnapshot? snapshot, bool notify, string repoKey, string? failure)
    {
        ...
        {
            // The saved token was rejected (revoked/expired): drop it and ask the
            // user to sign in again. OAuth-app tokens have no refresh token to try.
            CredentialStore.Clear();
            LastError = Loc.Get("Auth_Expired");
        }
        if (notify && _primed)
