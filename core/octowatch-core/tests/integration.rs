...
#[test]
fn gets_pull_request_detail() {
    let c = client();
    let r = repo("cli", "cli");
    let prs = c.list_pull_requests(r.clone()).expect("pull requests");
    assert!(!prs.is_empty());
    let number = prs[0].number;

    let detail = c.get_pull_request(r, number).expect("pull detail");
    assert_eq!(detail.number, number);
    assert!(detail.html_url.contains("/pull/"));
    assert!(!detail.base_branch.is_empty());
}
